...
subroutine primitive(dt)
  ...
  do k = ku1, ku2
     ...
     do j = ju1, ju2
        ...
        do i = iu1, iu2
           ...
           if(forcing) qin(i,j,k,ip)=half*qin(i,j,k,ir)**2
#endif
  ...
  !    write (*,*) 'Pressure:',mype,qin(1:nx,1,nz,5)
  ! endif
...
subroutine slope(bfc, dq, dbfc)
  use params
  use variables, only: qin,z
  use oacc_params
  use atmosphere , only: zbuffer
  implicit none
  ...
  ! well-balanced scheme variables
  real(dp)    :: r,p,q0im1,q0ip1,q0kp1,q0km1
  ...
  if (slope_type < 3) then
     ...
     do k = klo, khi
        ...
        do j = jlo, jhi
           ...
           do i = ilo, ihi
              do ivar = 1, nvar
                 r=qin(i,j,k,1) ; p=qin(i,j,k,5)
                 if (ivar==1) call getRhoFaces     (r,p,q0ip1,q0im1,gamma,dx)
                 if (ivar==5) call getPressureFaces(r,p,q0ip1,q0im1,gamma,dx)
                 if ((ivar/=1).and.(ivar/=5)) then
                    q0im1=qin(i,j,k,ivar) ; q0ip1=q0im1
                 endif
                 dlft = slope_type*(q0im1             - qin(i-1,j,k,ivar))
                 drgt = slope_type*(qin(i+1,j,k,ivar) - q0ip1            )
        ...
        do j = jlo, jhi
           ...
           do i = ilo, ihi
              do ivar = 1, nvar
                 ...
                 r=qin(i,j,k,1) ; p=qin(i,j,k,5)
                 if (ivar==1) call getRhoFaces     (r,p,q0kp1,q0km1,gamma,dz)
                 if (ivar==5) call getPressureFaces(r,p,q0kp1,q0km1,gamma,dz)
                 if (((ivar/=1).and.(ivar/=5)).or.(z(k)>zbuffer)) then
                    q0km1=qin(i,j,k,ivar) ; q0kp1=q0km1
                 endif
                 dlft = slope_type*(q0km1             - qin(i,j,k-1,ivar))
                 drgt = slope_type*(qin(i,j,k+1,ivar) - q0kp1            )
                 dcen = half*(dlft + drgt)/slope_type
                 dsgn = sign(one, dcen)
                 slop = min(abs(dlft), abs(drgt))
                 dlim = slop
        ...
        do j = jlo, jhi
           ...
           do i = ilo, ihi
              do ivar = 1, nvar
                 ...
                 dflll = (qin(i-1,j-1,k-1,ivar) - q0km1)/sqrt(dx**2+dy**2+dz**2)
                 dflml = (qin(i-1,j  ,k-1,ivar) - q0km1)/sqrt(dx**2+dz**2)
                 dflrl = (qin(i-1,j+1,k-1,ivar) - q0km1)/sqrt(dx**2+dy**2+dz**2)
                 dfmll = (qin(i  ,j-1,k-1,ivar) - q0km1)/sqrt(dy**2+dz**2)
                 dfmml = (qin(i  ,j  ,k-1,ivar) - q0km1)/dz
                 dfmrl = (qin(i  ,j+1,k-1,ivar) - q0km1)/sqrt(dy**2+dz**2)
                 dfrll = (qin(i+1,j-1,k-1,ivar) - q0km1)/sqrt(dx**2+dy**2+dz**2)
                 dfrml = (qin(i+1,j  ,k-1,ivar) - q0km1)/sqrt(dx**2+dz**2)
                 dfrrl = (qin(i+1,j+1,k-1,ivar) - q0km1)/sqrt(dx**2+dy**2+dz**2)

                 dfllm = (qin(i-1,j-1,k  ,ivar)-qin(i,j,k,ivar))/sqrt(dx**2+dy**2)
                 dflmm = (qin(i-1,j  ,k  ,ivar)-qin(i,j,k,ivar))/dx
                 dflrm = (qin(i-1,j+1,k  ,ivar)-qin(i,j,k,ivar))/sqrt(dx**2+dy**2)
                 dfmlm = (qin(i  ,j-1,k  ,ivar)-qin(i,j,k,ivar))/dy
                 dfmmm = (qin(i  ,j  ,k  ,ivar)-qin(i,j,k,ivar))
                 dfmrm = (qin(i  ,j+1,k  ,ivar)-qin(i,j,k,ivar))/dy
                 dfrlm = (qin(i+1,j-1,k  ,ivar)-qin(i,j,k,ivar))/sqrt(dx**2+dy**2)
                 dfrmm = (qin(i+1,j  ,k  ,ivar)-qin(i,j,k,ivar))/dx
                 dfrrm = (qin(i+1,j+1,k  ,ivar)-qin(i,j,k,ivar))/sqrt(dx**2+dy**2)

                 dfllr = (qin(i-1,j-1,k+1,ivar)-q0kp1)/sqrt(dx**2+dy**2+dz**2)
                 dflmr = (qin(i-1,j  ,k+1,ivar)-q0kp1)/sqrt(dx**2+dz**2)
                 dflrr = (qin(i-1,j+1,k+1,ivar)-q0kp1)/sqrt(dx**2+dy**2+dz**2)
                 dfmlr = (qin(i  ,j-1,k+1,ivar)-q0kp1)/sqrt(dy**2+dz**2)
                 dfmmr = (qin(i  ,j  ,k+1,ivar)-q0kp1)/dz
                 dfmrr = (qin(i  ,j+1,k+1,ivar)-q0kp1)/sqrt(dy**2+dz**2)
                 dfrlr = (qin(i+1,j-1,k+1,ivar)-q0kp1)/sqrt(dx**2+dy**2+dz**2)
                 dfrmr = (qin(i+1,j  ,k+1,ivar)-q0kp1)/sqrt(dx**2+dz**2)
                 dfrrr = (qin(i+1,j+1,k+1,ivar)-q0kp1)/sqrt(dx**2+dy**2+dz**2)
                 ...
                 if (ivar==5) call getPressureFaces(r,p,q0kp1,q0km1,gamma,dz)
                 if (((ivar/=1).and.(ivar/=5)).or.(z(k)>zbuffer)) then
                    q0km1=qin(i,j,k-1,ivar) ; q0kp1=qin(i,j,k+1,ivar)
                 endif
                 dfz  = half*(q0kp1-q0km1)
  ...
  do k = klo, khi
     ...
     do j = jlo, jhi
        ...
        do i = ilo, ihi
           ...
           if (balanced) then
              call getRhoFaces     (r,p,rm,rp,gamma,dx/2.d0)
              call getPressureFaces(r,p,pm,pp,gamma,dx/2.d0)
           endif
          ...
          if (Omega0 > zero) then
           ...
           qp(i,j,k,ir,1) = rp - drx
           qp(i,j,k,iu,1) = u - dux
           qp(i,j,k,iv,1) = v - dvx
           qp(i,j,k,iw,1) = w - dwx
           qp(i,j,k,ip,1) = pp - dpx
           qp(i,j,k,iA,1) = A
           qp(i,j,k,iB,1) = B - dBx
           qp(i,j,k,iC,1) = C - dCx
           ...
           qm(i,j,k,ir,1) = rm + drx
           qm(i,j,k,iu,1) = u + dux
           qm(i,j,k,iv,1) = v + dvx
           qm(i,j,k,iw,1) = w + dwx
           qm(i,j,k,ip,1) = pm + dpx
           qm(i,j,k,iA,1) = A
           qm(i,j,k,iB,1) = B + dBx
           qm(i,j,k,iC,1) = C + dCx
...
subroutine trace3d(bfc, dq, dbfc, qm, qp, qRT, qRB, qLT, qLB)
  use params
  use variables, only: qin, dt, x, y, z, Ex, Ey, Ez
  use oacc_params
  use atmosphere, only: balanced, zbuffer
  implicit none
     ...
     do j = jlo, jhi
        ...
        do i = ilo, ihi
           ...
           if (balanced) then
              rm=r ; rp=r ; pm=p ; pp=p
              if (z(k)<zbuffer) then
                 call getRhoFaces     (r,p,rm,rp,gamma,dz/2.d0)
                 call getPressureFaces(r,p,pm,pp,gamma,dz/2.d0)
              endif
           endif
           ...
           qp(i,j,k,ir,3) = rp - drz
           qp(i,j,k,iu,3) = u - duz
           qp(i,j,k,iv,3) = v - dvz
           qp(i,j,k,iw,3) = w - dwz
           qp(i,j,k,ip,3) = pp - dpz
           qp(i,j,k,iA,3) = A - dAz
           qp(i,j,k,iB,3) = B - dBz
           qp(i,j,k,iC,3) = CL     
           ...
           qm(i,j,k,ir,3) = rm + drz
           qm(i,j,k,iu,3) = u + duz
           qm(i,j,k,iv,3) = v + dvz
           qm(i,j,k,iw,3) = w + dwz
           qm(i,j,k,ip,3) = pm + dpz
           qm(i,j,k,iA,3) = A + dAz
           qm(i,j,k,iB,3) = B + dBz
           qm(i,j,k,iC,3) = CR     
...
subroutine update(fgodunov_pre)
  ...
  use oacc_params
  use atmosphere, only: beta,omegaEq
  use mpi_var
  implicit none
  ...
  !Vanishing fluxes at upper and lower boundaries are enforced.
#if MPI==1
  if (zposition==0        ) flux(:,:    ,1    ,1:5,3)=0.d0
  if (zposition==nzslice-1) flux(:,:    ,khi+1,1:5,3)=0.d0
  if (yposition==0        ) flux(:,1    ,:    ,1:5,2)=0.d0
  if (yposition==nyslice-1) flux(:,jhi+1,:    ,1:5,2)=0.d0
#else
  flux(:,:,1,1:5,3)=0.d0 ; flux(:,:    ,khi+1,1:5,3)=0.d0
  flux(:,1,:,1:5,2)=0.d0 ; flux(:,jhi+1,:    ,1:5,2)=0.d0
#endif

!  if ((zposition==nzslice-1).and.(yposition==0).and.(mype==15)) then
!     write (*,*) 'yo',mype,uin(1:nx,1,nz,4)
!     write (*,*) 'yo2',mype,fgodunov_pre(1:nx,1,nz,3)
!  endif
     ...
     do k = 1, khi
        do j = 1, jhi
           ...
           do i = 1, ihi
              ...
              ! Equatorial beta plane source terms
              lambda=(omegaEq+half*beta*y(j))*dt !"half" added for consistency with std beta def.
              ratio=(one-lambda**2)/(one+lambda**2)
              
              dsx= two*lambda*uin(i,j,k,3)/(1.d0+lambda**2)
              dsy=-two*lambda*uin(i,j,k,2)/(1.d0+lambda**2)
...
#endif
                 uin(i,j,k,iu)=uin(i,j,k,iu) + dfn2/(1.d0+lambda**2) + lambda/(1+lambda**2)*dfn3
                 uin(i,j,k,iv)=uin(i,j,k,iv) + dfn3/(1.d0+lambda**2) - lambda/(1+lambda**2)*dfn2
              enddo
...
subroutine constrained_transport
  ...
  !Vanishing x & y-emfs at upper and lower boundaries are enforced (Bz conservation).
#if MPI==1
  if (zposition==0) then
     emfx(:,:,1)=0.d0
     emfy(:,:,1)=0.d0
...
#else
  emfx(:,:,1)=0.d0 ; emfx(:,:,khi+1)=0.d0
  emfy(:,:,1)=0.d0 ; emfy(:,:,khi+1)=0.d0
#endif
#if MPI==1
  if (yposition==0) then
     emfx(:,1,:)=0.d0
