...
!! \details
!! Contains user_init(), get_eta(), get_csq(), get_P(), get_E(), getPressureFaces(), getRhoFaces()
!! \author
...
!###########################################################
!###########################################################
subroutine get_eta(etaVal,i,j,k)
  use params
  use variables , only : uin
  use atmosphere , only : Pcrit, eta_buffer, eta_body
  implicit none
  integer :: i,j,k
  real(dp) :: etaVal
